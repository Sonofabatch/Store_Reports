...
IF %found%==0 (
	SET strNumYstrdy=%1
	SET /a daynum=2
	ECHO Store Number %strNumYstrdy%:
:pochkloop
	IF %daynum% LEQ %tokentot% (
		ECHO Day is %daynum% or !daynum!
		ECHO Token total is %tokentot%
		ECHO Calling comparator from pochkloop(X !day!daynum!_prev!)
		PAUSE
		CALL :comparator X !day!daynum!_prev!
		SET /a daynum+=1
		ECHO pochklooping
		PAUSE
		GOTO :pochkloop
...
IF %strNumTdy%==%strNumYstrdy% (
	SET found=1
	ECHO Store Number %strNumTdy%:
	:strchkloop
	IF %daynum% LEQ %tokentot% (
		ECHO Day is %daynum% or !daynum!
		ECHO Token total is %tokentot%
		ECHO Calling comparator from strchkloop (!day%daynum%_cur! !day%daynum%_prev!)
		PAUSE
		CALL :comparator !day%daynum%_cur! !day%daynum%_prev!
		SET /a daynum+=1
		ECHO strchklooping
		PAUSE
		GOTO :strchkloop
...
:comparator
ECHO day%daynum% current data is %1
ECHO day%daynum% previous data is %2
IF %2==O (
	REM Check
